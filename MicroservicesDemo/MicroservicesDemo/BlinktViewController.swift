...
class BlinktViewController: UIViewController {
    ...
    var timer: Timer!
    var blinktCount = Common.Constants.defaultBlinktCount
    ...
    override func viewDidLoad() {
        ...
        blinktCounterField.text = String(Common.Constants.defaultBlinktCount)
        // default colors
    ...
    func postBlinkt() {
        ...
        SVProgressHUD.show(withStatus: Common.Dialogs.postingBlinkt + " \(blinktColor)...")
        
        let url = Common.Constants.urlIoTBlinktRandomWithAuth + "&color=\(blinktColor)"
        let params: [String:String] = [:]
        
        print("url: \(url), params: \(params), Authorization: Bearer \(String(describing: MASUser.current()?.accessToken))")
        
        MAS.post(to: url, withParameters: params, andHeaders: [:], completion:  { (response, error) in
            ...
            if (error != nil) {
                ...
                self.blinktCount = Common.Constants.defaultBlinktCount
                self.blinktCounterField.text = String(self.blinktCount)
    ...
    func buttonUp(sender: AnyObject) {
        ...
        blinktCount = Common.Constants.defaultBlinktCount
