...
class BeersTableViewController: UITableViewController {
    ...
    func getBeerList() {
        ...
        let url = Common.Constants.urlBeerList
        var params = ["auth":Common.Constants.lacAuthKey, "sysorder":"(updated_at:desc)"]
        if Common.Constants.BeerBlinktOn == true {
            params["blinkt"] = "true"
        }
        print("url: \(url), params: \(params), Authorization: Bearer \(String(describing: MASUser.current()?.accessToken))")
        
        MAS.getFrom(url, withParameters: params, andHeaders: nil, completion: { (response, error) in
...
//                print(jsonDataBase64 as Any)
            ...
            var url = Common.Constants.urlBeerList + "/\(beerId)?auth=\(Common.Constants.lacAuthKey)"
            let params = ["body-base64": jsonDataBase64!]
            if Common.Constants.BeerBlinktOn == true {
                url = Common.Constants.urlBeerList + "/\(beerId)?auth=\(Common.Constants.lacAuthKey)&blinkt=true"
            }
            print("url: \(url), params: \(params), Authorization: Bearer \(String(describing: MASUser.current()?.accessToken))")
            
            MAS.patch(to: url, withParameters: params, andHeaders: ["X-body-base64":"true"], completion: { (response, error) in
    ...
    func deleteBeer(beerId: String, checksum: String) {
        ...
        let url = Common.Constants.urlBeerList + "/\(beerId)"
        var params = ["auth":Common.Constants.lacAuthKey,"checksum":checksum]
        if Common.Constants.BeerBlinktOn == true {
            params["blinkt"] = "true"
        }
        print("url: \(url), params: \(params), Authorization: Bearer \(String(describing: MASUser.current()?.accessToken))")
        
        MAS.delete(from: url, withParameters: params, andHeaders: [:], completion:  { (response, error) in
